...
- :guilabel:`Image`: displays an image using a URL. The value cannot be edited manually, but a
  ...
  .. note::
     This works differently than selecting the :ref:`Image field <simple-fields/image>` directly, as
     the image is not stored in Odoo when using a :guilabel:`Text` field with the :guilabel:`Image`
     widget. For example, it can be useful if you want to save disk space.
...
.. note::
   The :guilabel:`Handle` widget displays a drag handle icon to move records manually in :ref:`List
   view <multiple-records/list>`.
...
- :guilabel:`Monetary`: it is similar to using the :ref:`Monetary field <simple-fields/monetary>`.
...
- :guilabel:`Priority`: displays star symbols instead of values, which can be used to indicate an
  ...
  :ref:`Priority field <simple-fields/priority>`, although, for the latter, four priority values are
  already predefined.
...
<simple-fields/selection>` with the :guilabel:`Priority` widget selected by default and four
...
:ref:`Selection <simple-fields/selection>`.
...
<general/form>`. This field type is a :ref:`File field <simple-fields/file>` with the
:guilabel:`Image` widget selected by default. Consequently, the :guilabel:`File`, :guilabel:`PDF
Viewer`, and :guilabel:`Sign` widgets have the same effects as described under :ref:`File
<simple-fields/file>`.
...
field <simple-fields/file>` with the :guilabel:`Sign` widget selected by default. Consequently, the
...
.. example::
   ...
   pointing at the *Contact* model. This allows many sales orders to be linked to the same contact
   (customer).
...
.. note::
   ...
   :ref:`Many2One field <relational-fields/many2one>`. One2Many relations do not exist
...
.. example::
   ...
   the *Contact* model. This allows the same user to be assigned to multiple tasks and multiple
   users to be assigned to the same task.
...
- :guilabel:`Tags`: users can select several values appearing in rounded shapes, also known as
  *tags*. This has the same effect as selecting the :ref:`Tags field <relational-fields/tags>`.
...
<relational-fields/many2many>` with the :guilabel:`Tags` widget selected by default. Consequently,
the :guilabel:`Checkboxes` and :guilabel:`Many2Many` widgets have the same effects as described
under :ref:`Many2Many <relational-fields/many2many>`.
